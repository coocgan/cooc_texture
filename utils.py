...
class TextureDataset(Dataset):
    ...
    def __init__(self, img_path, factor_scale, transform=None, cooc_tr=None, epoch_constant=2000,
        ...
        # Making the epoch constant in line with batch size
        epoch_constant = math.ceil(epoch_constant / opt.batchSize) * opt.batchSize
        ...
        self.Z_train = [0] * len(self.X_train)
    ...
    def __getitem__(self, index):
        ...
        try:
            if self.Z_train[index] == 0:
                cooc_gt, _ = cooc_utils.collect_cooc_batch(data_im_tensor.unsqueeze(0), gmm=self.im_gmm,
                                                           f_cooc=self.f_cooc)
                self.Z_train[index] = cooc_gt
        except:
            cooc_gt = self.Z_train[index]
